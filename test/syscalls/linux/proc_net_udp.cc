...
std::string DescribeFirstInetSocket(const SocketPair& sockets) {
  ...
  return StrFormat("fd:%d %8X:%4X", sockets.first_fd(), IP(addr), Port(addr));
}

std::string DescribeSecondInetSocket(const SocketPair& sockets) {
  const struct sockaddr* addr = sockets.second_addr();
  return StrFormat("fd:%d %8X:%4X", sockets.second_fd(), IP(addr), Port(addr));
}
...
bool FindBy(std::vector<UDPEntry> entries, UDPEntry* match,
...
bool FindByLocalAddr(std::vector<UDPEntry> entries, UDPEntry* match,
...
bool FindByRemoteAddr(std::vector<UDPEntry> entries, UDPEntry* match,
                      const struct sockaddr* addr) {
  uint32_t host = IP(addr);
  uint16_t port = Port(addr);
  return FindBy(entries, match, [host, port](const UDPEntry& e) {
...
PosixErrorOr<bool> FindByFD(std::vector<UDPEntry> entries, UDPEntry* match,
                            int fd) {
...
TEST(ProcNetUDP, Exists) {
  ...
  if (IsRunningOnGvisor()) {
    // Should be just the header since we don't have any udp sockets yet.
...
TEST(ProcNetUDP, FindMutualEntries) {
  ...
  EXPECT_TRUE(FindByLocalAddr(entries, &e, sockets->second_addr()))
      << DescribeSecondInetSocket(*sockets);
  EXPECT_TRUE(FindByRemoteAddr(entries, &e, sockets->second_addr()))
      << DescribeFirstInetSocket(*sockets);
}
...
TEST(ProcNetUDP, EntriesRemovedOnClose) {
  ...
  // still exist.
  EXPECT_FALSE(FindByLocalAddr(entries, &e, sockets->first_addr()))
      << DescribeFirstInetSocket(*sockets);
  EXPECT_TRUE(FindByLocalAddr(entries, &e, sockets->second_addr()))
      << DescribeSecondInetSocket(*sockets);
  ...
  EXPECT_FALSE(FindByLocalAddr(entries, &e, sockets->first_addr()))
      << DescribeFirstInetSocket(*sockets);
  EXPECT_FALSE(FindByLocalAddr(entries, &e, sockets->second_addr()))
      << DescribeSecondInetSocket(*sockets);
}

PosixErrorOr<std::tuple<std::unique_ptr<FileDescriptor>, int>>
BoundUDPSocket() {
  ASSIGN_OR_RETURN_ERRNO(std::unique_ptr<FileDescriptor> socket,
                         IPv4UDPUnboundSocket(0).Create());
  ASSIGN_OR_RETURN_ERRNO(
      int port, PortAvailable(0, AddressFamily::kIpv4, SocketType::kUdp,
                              /*reuse_addr=*/false));
  ...
  return std::make_tuple(std::move(socket), port);
}

TEST(ProcNetUDP, BoundEntry) {
  std::unique_ptr<FileDescriptor> socket;
  int port;
  std::tie(socket, port) = ASSERT_NO_ERRNO_AND_VALUE(BoundUDPSocket());
  std::vector<UDPEntry> entries =
      ASSERT_NO_ERRNO_AND_VALUE(ProcNetUDPEntries());
  UDPEntry e;
...
TEST(ProcNetUDP, BoundSocketStateClosed) {
  std::unique_ptr<FileDescriptor> socket;
  std::tie(socket, std::ignore) = ASSERT_NO_ERRNO_AND_VALUE(BoundUDPSocket());
  std::vector<UDPEntry> entries =
      ASSERT_NO_ERRNO_AND_VALUE(ProcNetUDPEntries());
  UDPEntry e;
  ASSERT_TRUE(ASSERT_NO_ERRNO_AND_VALUE(FindByFD(entries, &e, socket->get())));
