...
// Inode interface must be implemented by structs representing ext fs inodes.
// The inode stores all the metadata pertaining to the file (except for the
// file name which is held by the directory entry). Does NOT expose all fields,
// should be extended if need be.
//
// Some file systems (e.x. FAT) use the directory entry to store all this
...
type Inode interface {
	...
	// AccessTime returns the last access time in seconds since the epoch.
	...
	// ChangeTime returns the last change time in seconds since the epoch.
	// Shows when the file meta data (like permissions) was last changed.
	//
	// If InExtendedAttr is set, then this should NOT be used because the
	...
	// ModificationTime returns the last modification time in seconds since
	// the epoch. Shows when the file content was last modified.
	//
	...
	// DeletionTime returns the deletion time in seconds since the epoch.
	// Inodes are marked as deleted by writing to the underlying field. FS tools
	// can restore files until they are actually overwritten.
