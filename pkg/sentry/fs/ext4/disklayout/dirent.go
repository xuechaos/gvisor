...
var (
	...
	// ErrResolveViaInode is a special error value returned by Dirent.FileType()
	// to indicate that the dirent does not have information about the file type
	// and hence client will have to read inode struct to figure it out.
	ErrResolveViaInode = errors.New("file type should be read from Inode")
)
...
type Dirent interface {
	...
	// feature is set.
	//
	// If this errors out then the inode is has to be read to find the file type.
	FileType() (fs.InodeType, error)
}
