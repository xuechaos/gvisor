...
func TestTimestampSeconds(t *testing.T) {
	type timestampTest struct {
		// date is the exact date this test case tests. In format YYYY-MM-DD.
		date string
	...
	for _, test := range tests {
		t.Run(getTestName(test), func(t *testing.T) {
			got := fromExtraTime(get32BitTime(test), test.extraBits)
			if got != test.want {
				t.Errorf("Expected: %v (%s), Got: %v", test.want, test.date, got)
			}
		})
	}
}
