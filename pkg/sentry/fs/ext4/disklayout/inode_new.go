...
// the old size (128 bytes). The actual size of this struct should be
// determined using inode.ExtraInodeSize. Accessing any field here should
// be verified with the actual size. The extra space between the end of the
// inode struct and end of the inode record can be used to store extended attr.
//
...
// getSeconds decodes the extra time and constructs the larger seconds field.
func getSeconds(lo int32, extra uint32) int64 {
	return int64((extra&0x3)<<32) + int64(lo)
}

// getNanoSeconds extracts the nanoseconds part.
func getNanoSeconds(extra uint32) int64 {
	return int64(extra >> 2)
}
...
func (in *InodeNew) InodeSize() uint16 { return 128 + in.ExtraInodeSize }
...
func (in *InodeNew) ChangeTime() time.Time {
	...
	if in.ExtraInodeSize >= 8 {
		return time.FromUnix(getSeconds(in.ChangeTimeRaw, in.ChangeTimeExtra), getNanoSeconds(in.ChangeTimeExtra))
	}
...
func (in *InodeNew) ModificationTime() time.Time {
	...
	if in.ExtraInodeSize >= 12 {
		return time.FromUnix(getSeconds(in.ModificationTimeRaw, in.ModificationTimeExtra), getNanoSeconds(in.ModificationTimeExtra))
	}
...
func (in *InodeNew) AccessTime() time.Time {
	...
	if in.ExtraInodeSize >= 16 {
		return time.FromUnix(getSeconds(in.AccessTimeRaw, in.AccessTimeExtra), getNanoSeconds(in.AccessTimeExtra))
	}
