...
type Inode struct {
	...
	// operations.
	//
	// A reference is held on all Inodes for which nlink != 0.
	refs int64
...
func (i *Inode) init(fs *Filesystem, creds *auth.Credentials, mode uint16) {
	i.refs = 1
	i.mode = uint32(mode)
...
func (i *Inode) decRef() {
	if refs := atomic.AddInt64(&i.refs, -1); refs == 0 {
		if regfile, ok := i.impl.(*regularFile); ok {
			// This is unnecessary; it's mostly to simulate what tmpfs would
			// do.
			regfile.mu.Lock()
			regfile.data = nil
