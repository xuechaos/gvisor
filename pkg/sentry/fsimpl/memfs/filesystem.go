...
// checkDeleteLocked checks that an existing file named rp.Component() may be
// deleted in directory parentVFSD, then returns the vfs.Dentry for that file.
//
// Preconditions: Filesystem.mu must be locked. parentInode ==
// parentVFSD.Impl().(*Dentry).inode. parentInode.isDir() == true.
func checkDeleteLocked(rp *vfs.ResolvingPath, parentVFSD *vfs.Dentry, parentInode *Inode) (*vfs.Dentry, error) {
	if err := parentInode.checkPermissions(rp.Credentials(), vfs.MayWrite|vfs.MayExec, true); err != nil {
		return nil, err
	}
	pc := rp.Component()
...
func (i *Inode) open(rp *vfs.ResolvingPath, vfsd *vfs.Dentry, flags uint32, afterCreate bool) (*vfs.FileDescription, error) {
	ats := vfs.AccessTypesFromOpenFlags(flags)
	if !afterCreate {
...
func (fs *Filesystem) RmdirAt(ctx context.Context, rp *vfs.ResolvingPath) error {
	...
	_, err = checkDeleteLocked(rp, parentVFSD, parentInode)
	if err != nil {
		return err
	}
	// TODO: check for MS_RDONLY
...
func (fs *Filesystem) UnlinkAt(ctx context.Context, rp *vfs.ResolvingPath) error {
	...
	// TODO: check for MS_RDONLY
	// TODO: actually implement UnlinkAt
	return syserror.EPERM
}
