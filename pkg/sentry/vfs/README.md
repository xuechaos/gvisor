...
-   VirtualFilesystem: A reference is held on all Mounts that are actually
    mounted.
...
-   VFS works almost exclusively with pointers to Dentry (rather than DentryImpl
    ...
    inlining and escape analysis), reduces the size of VFS types, and allows
    pointers to be loaded and stored atomically using `sync/atomic`.
    Implementation-defined behavior is accessed via `Dentry.impl` when required.
